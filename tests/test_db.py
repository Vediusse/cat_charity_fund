...
def test_check_migration_file_exist():
    ...
    assert "alembic" in app_dirs, "В корневой директории не обнаружена папка `alembic`."
    ALEMBIC_DIR = BASE_DIR / "alembic"
    version_dir = [d.name for d in ALEMBIC_DIR.iterdir()]
    assert "versions" in version_dir, "В папке `alembic` не обнаружена папка `versions`"
